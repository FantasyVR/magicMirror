...
"""
def CRwithLLT(complianceMatrix, G, L, b, x0):
    m, n = L.shape
    ...
    while count < MaxCGIte:
        Sr = computeSv(complianceMatrix, G, L, r)
        rSr = sum(r * Sr)
        alpha = rSr / sum(Sp * Sp)
        x = x + alpha * p
        r_next = r - alpha * Sp
        if np.linalg.norm(r_next) < residual:
            break
        Sr_next = computeSv(complianceMatrix, G, L, r_next)
...
def solveWithSchurComplement(mass, p, prep, g, KK, l, c, cidx, iteration):
    ...
    dl = CRwithLLT(complianceMatrix, G, L, b, x0)
