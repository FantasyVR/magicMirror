...
def solveWithSchurComplement(mass, p, prep, g, KK, l, c, cidx, iteration):
    ...
    m = dim-2 +NC
    A_global = np.zeros((m, m),dtype=np.float64)
    A_global[0:dim-2, 0:dim-2] = A 
    A_global[dim-2:m,0:dim-2] = G.transpose()
    A_global[0:dim-2, dim-2:m] = G
    A_global[dim-2:m, dim-2:m] = complianceMatrix

    b_global = np.zeros(m, dtype=np.float64)
    b_global[0:dim-2] = u
    b_global[dim-2:m] = v 
    ...
    # Feed into AMGX
    A_global_sparse = sparse.csr_matrix(A_global)
    A_amgx.upload_CSR(A_global_sparse)
    b_amgx.upload(b_global)
    x_amgx.upload(x)
    solver_amgx.setup(A_amgx)
    solver_amgx.solve(b_amgx, x_amgx)
    x_amgx.download(x)
    print("pyamgx solution: ", x)
    print("scipy solution: ", splinalg.spsolve(A_global_sparse, b_global))
    x = x[0:dim-2]
    y = x[dim-2:m]
...
initRod()
initConstraint()
gui = ti.GUI('Stable Constrainted Dynamics with Nvidia AMGX solver')
pause = False
count = 0
frame = 0
while gui.running:
    ...
    # gui.show()
    filename = f'./amgx/frame_{frame:05d}.png'  # create filename with suffix png
    frame += 1
    if frame == 300:
        break
    gui.show(filename)
